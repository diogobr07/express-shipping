    ...
    return function()    {
        ...
        new Ajax.Request(url, {
            ...
            if(transport.status == 200)    {
                ...
                if(shipment_methods_found)  {
					var available_shipping_methods = data.shipping_method;
					new Ajax.Request('/index.php/onestepcheckout/ajax/save_available_shipping_methods',
						{
							parameters : { options : available_shipping_methods }
						}
					);
                    shipment_methods.update(available_shipping_methods);
                }
                payment_methods.replace(data.payment_method);
                totals.update(data.summary);
...
function addressPreview(templates, target) {
    ...
    if(payment_method){
		if(payment_method == 'ipgcore'){
			var payment_id = $$('#payment_form_ipgcore .img-container.select')[0].id.substr(3,2);
			if(payment_id == '15'){
				payment_img = $$('#payment_form_ipgcore .img-container.select')[0].cloneNode(true);
				payment_label = $$('.divPagForm-15 .ipagare-parcelas')[0].childElements()[2].innerHTML;
			}else{
				payment_img = $$('#payment_form_ipgcore .img-container.select')[0].cloneNode(true);
				payment_img.children[0].src = payment_img.children[0].src.replace(/[2-9]{1,2}x/, '1x');
				select_element = $$('.divPagForm-' + payment_id + ' .ipagare-parcelas select')[0];
				payment_label = select_element.options[select_element.selectedIndex].innerHTML;
			}
		}
    }

    var targetelem = $(target + '_personaldata').childElements()[1];
    if(targetelem){
        targetelem.update('');
        
		if(!customer){
			targetelem.insert("<span>"+ $('billing:firstname').value + ' ' + $('billing:lastname').value + "</span><br /><br />");
			targetelem.insert("<span>T: "+ $('billing:telephone').value + "</span><br /><br />");
		} else {
			targetelem.insert("<span>"+ customer.name + "</span><br /><br />");	
			targetelem.insert("<span>T:"+ customer.telephone + "</span><br /><br />");	
		}
		
		if($('billing:fax').value != ''){
			targetelem.insert("<span>C: "+ $('billing:fax').value + "<span><br /><br />");
		}
		
		if($('billing:email')){
			targetelem.insert("<span>CPF: "+ $('billing:taxvat').value + "<span><br /><br />");
			targetelem.insert("<span>"+ $('billing:email').value + "<span>");		
		}else{
			targetelem.insert("<span>"+ customer.taxvat + "</span><br /><br />");	
			targetelem.insert("<span>"+ customer.email + "</span>");	
		}
    }	
    ...
    if(targetelem){
        targetelem.update('');
		if(payment_method == 'ipgcore'){
			targetelem.insert(payment_img);
			targetelem.insert("<span class='payment_label'>" + payment_label + "</span>");
			if(payment_id == '15'){
				targetelem.insert("<p>O link para imprimir o boleto será gerado após a finalização da compra.</p>")
			}
		}
		targetelem.insert("<div class='clear'></div>");
    }

	/*var targetelem = $(target + '_otherinfo').childElements()[1];
	if(targetelem){
		targetelem.update('');
		
		var newsletter = $$('.onestepcheckout-enable-newsletter')[0].cloneNode(true);
		newsletter.childElements()[0].setAttribute('onclick', 'javascript:newsletterCheck(this)');
		newsletter.childElements()[1].removeAttribute('for');
		
		var terms_and_conditions = $$('.onestepcheckout-enable-terms')[0].cloneNode(true);
		terms_and_conditions.childElements()[0].setAttribute('disabled', 'disabled');
		terms_and_conditions.childElements()[1].removeAttribute('for');
		terms_and_conditions.childElements()[1].childElements()[0].removeAttribute('target');
		terms_and_conditions.childElements()[1].childElements()[0].setAttribute('onclick', 'javasctipt:reviewmodal.close();review=false;');
		
		targetelem.insert(newsletter);
		targetelem.insert(terms_and_conditions);
	}*/
